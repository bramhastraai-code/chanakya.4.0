...
let BlogService = class BlogService {
    ...
    async findAll(pageSize, pageNumber, sortBy, sortOrder, searchQuery) {
    ...
    async findActiveBlogs(pageSize, pageNumber, sortBy, sortOrder, searchQuery) {
        const limit = parseInt(pageSize);
        const skip = (parseInt(pageNumber) - 1) * limit;
    ...
    async findOneActive(id) {
        const blog = await this.blogModel
            ...
            .exec();
        if (!blog) {
            throw new common_1.NotFoundException(`Active blog with ID ${id} not found`);
        }
        return blog;
    }
};
